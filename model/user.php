...
class User
{
        ...
        if ($result = $this->db->dbSelect($sql)) {
            if ($row = $result->fetch_assoc()) {
                if ($row['Jelszo'] == md5($password)) {
                    ...
                    $_SESSION["felhnev"] = $row['Felhasznalonev'];
                    $_SESSION["id"] = $row['ID'];
    ...
    {
        $link = '<a href=" http://banki13.komarom.net/hbence/ ">banki13.komarom.net/bence</a>';
    ...
    public function updateuser($felhneve,$fullnev,$telefon,$emailcim){
        $response = "";
        $sql = "UPDATE felhasznalok SET Felhasznalonev = '".$felhneve."' , Nev = '".$fullnev."' , Telefon = '".$telefon."' , email = '".$emailcim."' WHERE ID = '".$_SESSION['id']."'";
